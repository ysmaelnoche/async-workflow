...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Select Employee</h1>
                    <p class="text-sm text-gray-600">Choose an employee to submit a proxy request for</p>
                </div>
                <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Available Employees ({{ $departmentEmployees->count() }})</h3>
            </div>
            ...
            <div class="overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" id="employee-list">
                        ...
                        <tr class="employee-row hover:bg-gray-50" data-search="{{ strtolower($employee->FirstName . ' ' . $employee->LastName . ' ' . $employee->Emp_No) }}">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $employee->Titles }} {{ $employee->FirstName }} {{ $employee->MiddleName }} {{ $employee->LastName }} {{ $employee->Suffix }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $employee->Emp_No }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500">{{ $employee->Email ?? 'No email' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500">{{ $employee->user->position ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('secretary.create-proxy-request', ['employee_id' => $employee->Emp_No]) }}" 
            ...
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">There are no employees available in your department for proxy requests.</p>
            </div>
            @endif
        </div>
    </div>
