                ...
                <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100 space-y-6">
                    ...
                    <div class="bg-red-100 border border-red-400 text-red-700 p-4 rounded mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-sm mt-2">
                            ...
                            <div><strong>Position:</strong> {{ Auth::user()->position }}</div>
                            <div><strong>Access Role:</strong> {{ Auth::user()->accessRole }}</div>
                            <div><strong>Department:</strong> {{ Auth::user()->department->dept_name ?? 'N/A' }}</div>
                            <div><strong>Dept Code:</strong> {{ Auth::user()->department->dept_code ?? 'N/A' }}</div>
                            <div><strong>Request Status:</strong> {{ $formRequest->status }}</div>
                            <div><strong>To Department:</strong> {{ $formRequest->toDepartment->dept_code ?? 'N/A' }}</div>
                            <div><strong>Can Take Action:</strong> <span class="{{ $canTakeAction ? 'text-green-600' : 'text-red-600' }}">{{ $canTakeAction ? 'TRUE' : 'FALSE' }}</span></div>
                            <div><strong>Can Evaluate:</strong> <span class="{{ $canEvaluate ? 'text-green-600' : 'text-red-600' }}">{{ $canEvaluate ? 'TRUE' : 'FALSE' }}</span></div>
                            <div><strong>Current URL:</strong> {{ request()->url() }}</div>
                            <div><strong>Route Name:</strong> {{ request()->route()->getName() ?? 'N/A' }}</div>
                        </div>
                    </div>
