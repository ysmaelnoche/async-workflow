...
use App\Http\Controllers\SupervisorAssignmentController; // Add SupervisorAssignmentController
...
Route::middleware(['auth', 'prevent.admin'])->group(function () {
    ...
    Route::get('/dashboard', function () {
        ...
        if ($user->position === 'Secretary' || $user->isSecretary()) {
            return redirect()->route('secretary.dashboard');
        }
        
        // Check if user is PFMO Head
        if ($user->department && $user->department->dept_code === 'PFMO' && $user->position === 'Head') {
            return redirect()->route('pfmo.dashboard');
        }
        
        // Default employee dashboard - pass the request properly
    ...
    Route::post('/approvals/{formRequest}/process', [ApprovalController::class, 'processApprovalAction'])
        ->name('approvals.process')
        ->middleware('can:process-approvals');

    // Requests / Forms Route (for all authenticated users) - Employee only
    Route::get('/track/{formId}', [RequestController::class, 'track'])
        ->name('track');

    // Approver Assignment Routes (for users with manage-approvers permission) - Employee only
    ...
    Route::post('/approver-assignments/search', [ApproverAssignmentController::class, 'search'])
        ->name('approver-assignments.search')
        ->middleware('can:manage-approvers');
...
Route::middleware(['auth'])->prefix('secretary')->name('secretary.')->group(function () {
    ...
    Route::get('/select-employee', [App\Http\Controllers\SecretaryController::class, 'selectEmployee'])->name('select-employee');
    Route::get('/create-proxy-request', [App\Http\Controllers\SecretaryController::class, 'createProxyRequest'])->name('create-proxy-request');
    Route::post('/store-proxy-request', [App\Http\Controllers\SecretaryController::class, 'storeProxyRequest'])->name('store-proxy-request');
});
